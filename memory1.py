...
def new_game():
    global CardsList,exposed,openedCard,clickCounter,turns
    CardsList=[i for i in range(8)]+[i for i in range(8)]
...
def mouseclick(pos):
    ...
    if clickCounter==0:
        ...
        if not (pos[0]//50 in openedCard):
            if CardsList[openedCard[-1]]!=CardsList[openedCard[-2]]:
...
def draw(canvas):
        ...
        for i in range(16):
            ...
            if exposed[i]:
                canvas.draw_text(str(CardsList[i]), [15+50*i,70], 40, "White")
